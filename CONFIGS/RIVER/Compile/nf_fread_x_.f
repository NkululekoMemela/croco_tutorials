      integer*4 function nf_fread_x  (ncid, varid, type)
      implicit none
     ...
     &        i
      ...
      count=imax-imin+1
      do i=1,Lm+2
         A(i) = i + start - 1 + 0.5D0*horiz_type
      enddo
      ierr=nf_get_vara_double (ncid, varid, start, count, A)
      nf_fread_x=ierr
     ...
     &             'nf_get_vara netCDF error code =', ierr
      ...
      integer*4 function nf_fwrite_x (ncid, varid, type)
      implicit none
      integer*4  LLm,Lm,MMm,Mm,N, LLm0,MMm0
      parameter (LLm0=40,   MMm0=80,   N=20)
      parameter (LLm=LLm0,  MMm=MMm0)
     ...
     &        i
      ...
      do i=1,Lm+2
         A(i) = i + start - 1 + 0.5D0*horiz_type
      enddo
      ierr=nf_put_vara_double (ncid, varid, start, count, A)
      nf_fwrite_x=ierr
      if (ierr.ne.nf_noerr) then
