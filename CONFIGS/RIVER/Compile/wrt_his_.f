      subroutine wrt_his
      integer*4 ierr,  record, lstr, lvar, lenstr, type
     &   , start(2), count(2), nf_fwrite, cff,ik
     &   , ibuff(4)
      real eps
      parameter (eps=1.D-20)
      real stf_cff
      parameter(stf_cff=86400.D0/0.01D0)
      integer*4 tile, itrc,i,j,k
...
!$AGRIF_END_DO_NOT_TREAT
      real work(0:Lm+1+padd_X,-1:Mm+2+padd_E,0:N)
      common /work3d/ work
      real workr(0:Lm+1+padd_X,-1:Mm+2+padd_E,1:N)
      common /work3d_r/ workr
      real work2d(0:Lm+1+padd_X,-1:Mm+2+padd_E)
      common /work2d/ work2d
      real work2d2(0:Lm+1+padd_X,-1:Mm+2+padd_E)
     ...
     &        record,r2dvar,type)
      ...
      if (wrthis(indxBustr)) then
         work2d=bustr*rho0
         call fillvalue2d(work2d,ncidhis,hisBustr,indxBustr,
     &        record,u2dvar,type)
      endif
      if (wrthis(indxBvstr)) then
         work2d=bvstr*rho0
         call fillvalue2d(work2d,ncidhis,hisBvstr,indxBvstr,
     &        record,v2dvar,type)
      endif
     ...
     &        record,r3dvar,type)
      ...
      if (wrthis(indxbvf)) then
         work=bvf
         call fillvalue3d_w(work,ncidhis,hisbvf,indxbvf,
     &           record,w3dvar,type)
      endif
