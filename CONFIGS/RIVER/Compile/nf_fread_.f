      integer*4 function nf_fread  (A, ncid, varid, record, type)
      implicit none
     ...
     &          , isalt
      ...
      real A(0:Lm+1+padd_X,-1:Mm+2+padd_E,N+1)
      integer*4 ncid,  type,   vert_type,  imin, imax,  start(4)
     ...
     &             'nf_get_vara netCDF error code =', ierr
      ...
      integer*4 function nf_fwrite (A, ncid, varid, record, type)
      implicit none
      integer*4  LLm,Lm,MMm,Mm,N, LLm0,MMm0
      parameter (LLm0=40,   MMm0=80,   N=20)
      parameter (LLm=LLm0,  MMm=MMm0)
     ...
     &                'illegal grid type', type
      ...
      do k=1,count(3)
        ...
        enddo
      enddo
      ierr=nf_put_vara_double (ncid, varid, start, count, buff(1))
      nf_fwrite=ierr
      if (ierr.ne.nf_noerr) then
