      subroutine get_initial (req_rec, tindx)
      implicit none
     ...
     &                                ,   vname
      ...
      real time_scale
      integer*4 itrc
      integer*4 ncid, req_rec, tindx, record, max_rec,
     ...
     &             ''' in netCDF file ''',  ininame(1:lstr), '''.'
           ...
           write(stdout,2) vname(1,indxTime)(1:lvar)
     ...
     &         'read time and record indices.'
        ...
        nrecrst=0
        nrechis=0
         write(stdout,'(/1x,2A,G12.4,1x,A,I4)')
     &          'GET_INITIAL -- ',
     ...
     &                                      'record =', record
      ...
      if (ierr .eq. nf_noerr) then
        ierr=nf_fread (zeta(0,-1,1), ncid, varid,
     &                                         record, r2dvar)
        if (ierr .ne. nf_noerr) then
           write(stdout,2) vname(1,indxZ)(1:lvar)
     &                                ,record,ininame(1:lstr)
          goto 99
        endif
      else
     ...
     &                                ,ininame(1:lstr)
      ...
      if (ierr .eq. nf_noerr) then
        ierr=nf_fread (ubar(0,-1,1), ncid, varid,
     &                                         record, u2dvar)
        if (ierr .ne. nf_noerr) then
           write(stdout,2) vname(1,indxUb)(1:lvar)
     &                                  , record, ininame(1:lstr)
          goto 99
        endif
      else
     ...
     &                                ,ininame(1:lstr)
      ...
      if (ierr .eq. nf_noerr) then
        ierr=nf_fread (vbar(0,-1,1), ncid, varid,
     &                                         record, v2dvar)
        if (ierr .ne. nf_noerr) then
           write(stdout,2) vname(1,indxVb)(1:lvar)
     &                                , record, ininame(1:lstr)
          goto 99
        endif
      else
     ...
     &                                ,ininame(1:lstr)
      ...
      if (ierr .eq. nf_noerr) then
        ierr=nf_fread (u(0,-1,1,tindx), ncid, varid,
     &                                         record, u3dvar)
        if (ierr .ne. nf_noerr) then
           write(stdout,2) vname(1,indxU)(1:lvar)
     &                                 , record, ininame(1:lstr)
          goto 99
        endif
      else
     ...
     &                                ,ininame(1:lstr)
      ...
      if (ierr .eq. nf_noerr) then
        ierr=nf_fread (v(0,-1,1,tindx), ncid, varid,
     &                                         record, v3dvar)
        if (ierr .ne. nf_noerr) then
           write(stdout,2) vname(1,indxV)(1:lvar)
     &                                 , record, ininame(1:lstr)
          goto 99
        endif
      else
     ...
     &                                ,ininame(1:lstr)
      ...
      do itrc=1,NT
        ...
        if (ierr .eq. nf_noerr) then
          ierr=nf_fread (t(0,-1,1,tindx,itrc), ncid, varid,
     &                                                record, r3dvar)
          if (ierr .ne. nf_noerr) then
             write(stdout,2) vname(1,indxV+itrc)(1:lvar)
     &                                        , record, ininame(1:lstr)
            goto 99
          endif
        else
     ...
     &                                    ininame(1:lstr)
      ...
      if (ierr .eq. nf_noerr) then
        ierr=nf_fread (hbls(0,-1,1), ncid, varid,
     &                                         1, r2dvar)
        if (ierr .ne. nf_noerr) then
           write(stdout,2) vname(1,indxHbl)(1:lvar)
     &                                   , record, ininame(1:lstr)
          goto 99
        endif
      else
         write(stdout,4) vname(1,indxHbl)(1:lvar)
     &                                , ininame(1:lstr)
      endif
      ierr=nf_close(ncid)
