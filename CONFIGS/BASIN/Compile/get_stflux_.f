     ...
     &                                ,   vname
      ...
      real cff
      integer*4 itrc, i,ierr, lstr,lvar,lenstr, nf_fread, advance_cycle
     ...
     &           stf_cycle(itrc), stf_ncycle(itrc), stf_rec(itrc))
        if (may_day_flag.ne.0) return
        itstf(itrc)=2
        stf_time(1,itrc)=-1.D+20
        stf_time(2,itrc)=-1.D+20
        stf_scale(itemp)=1.D0/(rho0*Cp)
      endif
  1   i=3-itstf(itrc)
       cff=time+0.5D0*dt
     ...
     &                      stf_ncycle(itrc), stf_rec(itrc))
        if (ierr .ne. 0) goto 7
        ierr=nf_get_var1_double(ncidfrc, stf_tid(itrc),
     &                             stf_rec(itrc), cff)
        if (ierr .ne. nf_noerr) then
        ...
        if (stf_time(itstf(itrc),itrc).eq.-1.D+20)
     &      stf_time(itstf(itrc),itrc)=stf_time(i,itrc)
        if (lstfgrd(itrc).eq.1) then
          ierr=nf_fread(stflxg(0,0,i,itrc), ncidfrc,
     &                     stf_id(itrc), stf_rec(itrc), r2dvar)
        else
          ierr=nf_get_var1_double(ncidfrc, stf_id(itrc),
     &                     stf_rec(itrc), stflxp(i,itrc))
       ...
       itstf(itrc)=i
        write(stdout,'(6x,A,1x,I2,1x,A,1x,g12.4,1x,I4)')
  ...
  99  may_day_flag=2
      ...
      implicit none
      integer*4 Istr,Iend,Jstr,Jend, itrc, i,j, it1,it2
      real cff, cff1, cff2
      integer*4  LLm,Lm,MMm,Mm,N, LLm0,MMm0
      parameter (LLm0=60,   MMm0=50,   N=10)
      parameter (LLm=LLm0,  MMm=MMm0)
