      integer*4 function nf_fread_y  (ncid, varid, type)
      implicit none
      ...
      real A(Mm+2)
      integer*4 ncid,  type,   vert_type, imin, imax,  start,
     ...
     &        j
      ...
      count=jmax-jmin+1
      do j=1,Mm+2
         A(j) = j + start - 1 + 0.25D0*horiz_type
      enddo
      ierr=nf_get_vara_double (ncid, varid, start, count, A(1))
      nf_fread_y=ierr
     ...
     &             'nf_get_vara netCDF error code =', ierr
      ...
      integer*4 function nf_fwrite_y (ncid, varid, type)
      implicit none
      integer*4  LLm,Lm,MMm,Mm,N, LLm0,MMm0
      parameter (LLm0=60,   MMm0=50,   N=10)
      parameter (LLm=LLm0,  MMm=MMm0)
     ...
     &        j
      ...
      do j=1,Mm+2
         A(j) = j + start - 1 + 0.25D0*horiz_type
      enddo
      ierr=nf_put_vara_double (ncid, varid, start, count, A(1))
      nf_fwrite_y=ierr
      if (ierr.ne.nf_noerr) then
