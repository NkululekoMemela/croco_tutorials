     ...
     &                                ,   vname
      real cff
      integer*4 i,ierr, lstr,lvar, lenstr, nf_fread, advance_cycle
      integer*4 nf_byte
      integer*4 nf_int1
      integer*4 nf_char
      integer*4 nf_short
  ...
  1   i=3-itsrf
       cff=time+0.5D0*dt
        ...
        if (ierr.ne.0) goto 7
        ierr=nf_get_var1_double(ncidfrc, srf_tid, srf_rec, cff)
        if (ierr .ne. nf_noerr) then
          write(stdout,6) 'srf_time', srf_rec
          goto 99
        endif
        srf_time(i)=cff*day2sec+srf_cycle*srf_ncycle
        if (srf_time(itsrf).eq.-1.D+20) srf_time(itsrf)=srf_time(i)
        if (lsrfgrd.eq.1) then
          ierr=nf_fread (srflxg(0,0,i), ncidfrc, srf_id,
     &                                             srf_rec, r2dvar)
        else
          ierr=nf_get_var1_double (ncidfrc,srf_id,srf_rec,srflxp(i))
        endif
        if (ierr .ne. nf_noerr) then
       ...
       itsrf=i
        write(stdout,'(6x,A,1x,A,1x,g12.4,1x,I4)')
     &           'GET_SRFLUX --',
  ...
  99  may_day_flag=2
     ...
     &           jul_off=2440000.D0)
      ...
      common /Z0B_VAR/zob
      integer*4 Istr,Iend,Jstr,Jend, i,j, it1,it2
