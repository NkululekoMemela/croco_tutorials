     ...
     &                      )
      ...
      parameter (ntrc_diaeddy=0)
      parameter (ntrc_surf=0)
      real wrk   (Istr-2:Iend+2    )
      real wrk2  (Istr-2:Iend+2    )
     ...
     &                                  *om_r(i,j)*on_r(i,j)
      ...
      if (istr.eq.1 .and. jend.eq.Mm) then
        do k=0,N
          We(0,Mm+1,k)=We(1,Mm,k)
        enddo
      endif
      if (iend.eq.Lm .and. jstr.eq.1) then
        do k=0,N
          We(Lm+1,0,k)=We(Lm,1,k)
        enddo
      endif
      if (iend.eq.Lm .and. jend.eq.Mm) then
        do k=0,N
          We(Lm+1,Mm+1,k)=We(Lm,Mm,k)
        enddo
      endif
      return
      end
