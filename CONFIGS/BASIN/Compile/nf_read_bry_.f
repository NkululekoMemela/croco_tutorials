      integer*4 function nf_read_bry_NS (A, ncid, varid, record, type)
      implicit none
      ...
      real A(0:Lm+1+padd_X,N)
      integer*4 ncid, varid, record, type,  i,j,k, shft, ierr
      integer*4 xtype, ndims, dimid(8), natts
      character*12 vname
      integer*4 nf_byte
      integer*4 nf_int1
      integer*4 nf_char
      ...
      common /zzz/ buff
      integer*4 imin,imax,jmin,jmax, start(4), count(4),
     &                            vert_type, horiz_type
      ...
      if (ierr.eq.0) then
        ...
        start(3)=start(4)
        count(3)=count(4)
        ierr=nf_get_vara_double (ncid, varid, start, count, buff)
     ...
     &                          'illegal grid type', type
      ...
      integer*4 function nf_read_bry_EW (A, ncid, varid, record, type)
      implicit none
      integer*4  LLm,Lm,MMm,Mm,N, LLm0,MMm0
      parameter (LLm0=60,   MMm0=50,   N=10)
      parameter (LLm=LLm0,  MMm=MMm0)
